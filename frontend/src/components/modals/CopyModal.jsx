...
function CopyModal({ data, onClose }) {
    ...
    const handleCopy = async (destinationId) => {
        ...
        for (const itemId of data) {
            try {
                await copyItemApi(itemId, destination, true)
            } catch (error) {
                allSuccess = false
        ...
        if (allSuccess) {
            const message = data.length > 1 ? "Items copied successfully" : "Item copied successfully"
            ...
            >
                <div ref={modalRef} className="model-wrapper">
                    ...
                    <Modal.Body className="h-modal-lg p-0">
                        <CustomScroll className="copy-modal-body" showBottomBlur={false} showTopBlur={false}>
                            ...
                            <div className="share-user-container share-user-container-move-box">
                                ...
                                {!loading && folders.map(folder => {
                                    const isBeingCopied = data.includes(folder._id);
